...
- (NSString *)createDirectoryIfNULL
{
    ...
    if (![self directoryExists:directory])
    {
        ...
        {
            [NSException
                 raise:@"FailedToCreateCacheDirectory"
                format:@"Failed to create a directory for the cache at '%@'", directory];
        }
    }
    return directory;
}
...
- (instancetype)initInstance
{
    if (self = [super init])
    {
        ...
        _storePath = [[NSHomeDirectory() stringByAppendingPathComponent:@"Documents"]
            stringByAppendingString:@"/data-caches/"];
        _group = DEFAULT_GROUP_VALUE;
...
+ (instancetype)sharedFileCacheStore
{
    ...
    dispatch_once(&predicate,
                  ^(void) { fileCacheStore = [[FileCacheStore alloc] initInstance]; });
